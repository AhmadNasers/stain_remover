"""
Click Detection Module (Picamera2 version)
...
class ClickDetector:
    def __init__(self):
        self.boundaries = []
        self.window_name = "Click Detection (PiCam)"
    ...
    def mouse_callback(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            mapped = self.approximated_point(x, y)
            if mapped is not None:
                sx, sy = mapped
                print(f"Clicked pixel: ({x}, {y}) -> steps: ({sx}, {sy})")
            else:
                print(f"Clicked pixel: ({x}, {y}) -> steps: (N/A)")
    ...
    def approximated_point(self, x, y):
        ...
        x_steps, y_steps = mapped[0, 0]
        return int(round(x_steps)), int(round(y_steps))
    ...
    def draw_click_result(self, frame):
        ...
        if elapsed_seconds < 3.0:
            ...
            cv2.putText(
                frame,
                result_text,
                (x - text_width // 2, y + text_offset_y),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.8,
                result_color,
                2,
            ...
            (big_w, big_h), _ = cv2.getTextSize(result_display, cv2.FONT_HERSHEY_SIMPLEX, 2, 4)
            text_x = (frame.shape[1] - big_w) // 2
            text_y = 60

            cv2.rectangle(
                frame,
                (text_x - 20, text_y - big_h - 10),
                (text_x + big_w + 20, text_y + 10),
                (0, 0, 0),
                -1,
            )
            cv2.rectangle(
            ...
            cv2.putText(frame, result_display, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 2, result_color, 4)
    ...
    def draw_instructions(self, frame):
        ...
        instructions = [
            "CLICK DETECTION MODE (PiCamera2)",
    ...
    def _init_camera(self):
        ...
        # Source: image pixel coordinates (must match your preview_size and flips behavior)
        ...
        self.H, mask = cv2.findHomography(self.img_pts, self.step_pts, method=0)
        if self.H is None:
    ...
    def run(self):
        ...
        print("Click Detection Mode Started (PiCamera2)")
